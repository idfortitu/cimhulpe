...
' Ajouté : FilterFunc (tentative pour réparer l'erreur sur le .Where)
' L'appelant doit (enfin, peut) mettre dans FilterFunc une callback filtre(item as T, filtre as String) As Boolean
...
Public Class SortableBindingList(Of T)
    ...
    'Private Delegate Function FonctionPourFiltrer(item As Object, filtre As String) As Boolean

    'Public FilterFunc As FonctionPourFiltrer

    '    Private _filterFunc As Func(Of Object, String, Boolean)
    ...
    Private Function FiltrerUnElem(item As Object)
        Return FilterFunc(item, _filter)
    End Function

    'Public FilterFunc As Func(Of T, String, Boolean) = Function(a, b) True  ' filtre renvoie vrai par défaut
    ...
    Public Sub New(list As IEnumerable(Of T))
        ...
        ' filtre par défaut (se base sur ToString)
        FilterFunc = Function(i As Object, f As String) i.ToString.Contains(f)

    End Sub

    'Public Function FilterFunc(Of t)() As Boolean
    'End Function
    ...
    Protected Sub UpdateFilter()
        ...
        Try
            ...
            ' ligne changée parce que Overload resolution failed (vieux code où FilterFunc est autre chose)
            'If Not String.IsNullOrEmpty(_filter) Then filtered = filtered.Where(FilterFunc)
            If Not String.IsNullOrEmpty(_filter) Then filtered = filtered.Where(AddressOf FiltrerUnElem).AsQueryable
